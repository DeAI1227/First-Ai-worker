...
class HttpFetcherTests(unittest.TestCase):
    def test_http_fetcher_parses_fake_html(self):
        task = make_task(scope="industry", scope_name="??", stock_code="6230", stock_name="???", source_mode="http")
        state = {"run_errors": []}
        response = Mock()
    ...
    def test_http_fetcher_extracts_paragraph_content_without_article(self):
        task = make_task(scope="macro", scope_name="???", source_mode="http")
        state = {"run_errors": []}
        response = Mock()
        response.raise_for_status.return_value = None
        ...
        self.assertEqual(item["source_type"], "http")
        self.assertIn("?????? article ???????????", item["content"])
        self.assertGreater(len(item["content"].strip()), 0)

    def test_http_fetcher_request_failure_records_error(self):
        task = make_task(scope="industry", scope_name="??", source_mode="http")
        state = {"run_errors": []}
    ...
    def test_source_registry_can_resolve_http_urls_from_stock_source_rules(self):
        state = {
            ...
            "target_stock_name": "???",
            "source_rules": build_stock_source_rules("2330", "???"),
        }
        with patch.object(source_registry, "fetch_http_sources", return_value=[{"source_type": "http"}]) as mocked_http, patch.object(
            source_registry, "fetch_mock_sources", return_value=[{"source_type": "mock"}]
        ):
    ...
    def test_http_fetcher_follows_yahoo_stock_news_list_links(self):
        task = make_task(scope="stock", scope_name="???", stock_code="2330", stock_name="???", source_mode="http")
        state = {"run_errors": []}
        ...
        <html lang="zh-TW">
          ...
          <body>
            <a href="https://tw.stock.yahoo.com/news/article-one-1.html">???</a>
            <a href="/news/article-two-2.html">???</a>
            <a href="https://example.com/not-news">????</a>
          </body>
        </html>
        """
        ...
        self.assertIn("????????", raw_sources[1]["title"])
