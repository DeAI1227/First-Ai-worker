...
→ GitHub runner executes local pipeline segments
→ LangGraph Collector
→ ingestion
→ Supabase staging
→ promotion
...
- `daily-core`：07:00 Asia/Taipei
- `stock-pipeline`：07:20 Asia/Taipei
- `three-day-refresh`：07:40 Asia/Taipei
...
- 直接執行本地 pipeline script
- 寫入 Supabase staging / production

目前 workflow 已改成在 GitHub runner 裡直接執行，而不是透過 Render 接一個超長同步請求。
...
- `FIRECRAWL_BASE_URL`
- `FIRECRAWL_API_KEY`
- workflow log 裡卡在哪一個 batch
- `pipeline_report`、`batch_report`、`promotion_report`
...
[ ] GitHub Actions 可以直接跑本地 pipeline script
